...
module.exports = (Dependency) => {
  ...
  class Copacetic extends EventEmitter {
     ...
     * to become healthy. If retries is not specific then copacetic will
     * keep re-checking indefinitely until the dependency(s) become healthy.
     * @example
     * // wait indefinitely
    ...
    waitFor (opts = {}) {
      ...
      if (opts.dependencies) {
        _opts.dependencies = dependencies.map((d) => ({
          name: d.name,
          retries: d.retries || 0,
          maxDelay: d.maxDelay
        }))
